...
function App() {
  ...
  useEffect(() => {
    ...
    // Delay check slightly so landing dashboard renders cleanly first
  ...
  const startUpdate = async () => {
    ...
    try {
      ...
      // Delay relaunch slightly for feedback visibility
      setTimeout(async () => {
        try {
          await relaunch();
        } catch (err) {
    ...
    <div className="min-h-screen bg-[#070b14] text-[#eceff4] font-sans antialiased overflow-x-hidden p-6 select-none relative">
      ...
      {showUpdateModal && updateAvailable && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-5 animate-slide-in backdrop-blur-xl relative">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">A new version of PortPurge is ready for install</p>
              </div>
            </div>

            {/* Version Comparison */}
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/40 border border-slate-800/60 font-mono text-xs">
              ...
              <div className="space-y-0.5 text-right">
                ...
                <span className="text-indigo-400 font-bold">v{updateAvailable.version}</span>
              </div>
            </div>

            {/* Release Notes */}
            <div className="space-y-2">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Release Notes</span>
              <div className="max-h-40 overflow-y-auto p-4 rounded-xl bg-slate-950/60 border border-slate-800/80 text-xs text-slate-300 leading-relaxed font-mono whitespace-pre-wrap">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              {!isDownloading && (
                <>
                  ...
                  <button
                    onClick={startUpdate}
                    className="px-5 py-2.5 text-xs font-bold bg-gradient-to-tr from-indigo-500 to-violet-600 hover:from-indigo-450 hover:to-violet-550 text-white rounded-xl shadow-lg shadow-indigo-500/10 hover:shadow-indigo-500/20 transition cursor-pointer flex items-center gap-1.5"
                  >
